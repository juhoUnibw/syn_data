                ...
                for ix,val in train_class_df.iterrows():
                    for col in columns:
                        ...
                        if(col != class_col and (neighbor_df.dtype == np.int64 or neighbor_df.dtype == np.int32)): 
                            ...
                            ind = np.digitize(x, bins=centers , right=True) # returns indices of bins the diffused values fall into
                            x = np.array([bin_val[i] for i in ind]) # original values are taken from bins that contain nn of diffused values >> discretization
    ...
    def sample(self, data,real,class_col):
        ...
        if(not isinstance(class_col,type(None)) and len(np.unique(train[class_col]))<=20):
            ...
            try:
                ...
                temp_data = surrogate_data.sample(frac=self.smpl_frac, replace='True')
            synth_data = pd.concat([synth_data,temp_data],axis=0) 
        for x in train.columns:
