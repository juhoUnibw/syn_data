...
def gen(train_set, smpl_frac, gen_meth, disc_feat_names, cont_feat_names, class_var):
    ...
    # converts a list of integers to a list of strings >> required by transformers library (base.py)
    ...
    # initializes synthesis method with meta information and generates random sample
    def sdv_gen(gen_meth, metadata, train_set, smpl_size):
        ...
        if gen_meth == 'ctgan':
            synthesizer = CTGANSynthesizer(
                ...
                batch_size = 8, #500
                pac = 8) # requirement > batch_size % pac = 0
        if gen_meth == 'tvae':
    ...
    for cl in train_set[class_var].unique():
        ...
        gen_data_cl = sdv_gen(gen_meth, metadata, train_set_cl, cl_size) # generation per class >> Dankar, Hittmeir, and Patki do not mention how to deal with that
