...
def gen(trainSet, disc_feat_names, smpl_frac):
    ...
    model = Tabula(llm='distilgpt2', experiment_dir = "IEEE", batch_size=32, epochs=200, categorical_columns = disc_feat_names) # 100/400 epochs for the smallest models in original paper. 200 chosen because same amount for GReaT.
    model.model.load_state_dict(torch.load("pretrained-model/model.pt", map_location=torch.device('cpu')), strict=False) # pre-trained model from paper (trained on Intrusion dataset). map_location=torch.device('cpu') only if no GPU available
    model.fit(trainSet)
